/*___________________________________________________________________________
  WARNING: script to drop OrChem objects from Oracle schema.
  Used for UNIT TESTING only ("disposable" schema).
  ___________________________________________________________________________*/
...
begin
    ...
    dbms_output.put_line ( 'Dropping all OrChem objects schema "&1" ..');

    for jav in ( select name from user_java_classes where name like '%orchem%')
    loop 
        begin
           dbms_java.dropjava(jav.name);
        exception 
