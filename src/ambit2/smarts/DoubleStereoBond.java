...
public class DoubleStereoBond extends SMARTSBond
{
	...
	public int stereoParameter = 0; //contains the absolute stereo information of the query 
    ...
    public boolean matches(IBond bond)
    {
            ...
            if (stereoParameter > 0)
            {
                    ...
                    case SmartsConst.BT_CIS:
                            if (bond.getStereo() != IBond.Stereo.Z) // SmartsConst.ABSOLUTE_CIS)
                                    return(false);
                            break;
                    case SmartsConst.BT_CISUNSPEC:
                            if (bond.getStereo() == IBond.Stereo.NONE )// 0)
                                    return(true);
                            if (bond.getStereo() != IBond.Stereo.Z) //SmartsConst.ABSOLUTE_CIS)
                                    return(false);
                            break;
                    case SmartsConst.BT_TRANS:
                            if (bond.getStereo() != IBond.Stereo.E) //SmartsConst.ABSOLUTE_TRANS)
                                    return(false);
                            break;
